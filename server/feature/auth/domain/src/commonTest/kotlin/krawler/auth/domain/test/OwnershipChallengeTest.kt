...
import krawler.auth.domain.OwnershipChallengeUpdateEvent
...
class OwnershipChallengeTest {
    ...
    fun `should return AttemptsExceeded if current attempts are equal to max`() {
        ...
        // THEN
        assertEquals(OwnershipChallengeResult.AttemptsExceeded, result.returning)
        assertNull(result.event, "No event should be emitted when attempts are exceeded")
    }
    ...
    fun `should return BattleBeforeTask if battle ends before timeframe`() {
        ...
        // THEN
        assertEquals(OwnershipChallengeResult.BattleBeforeTask, result.returning)
    ...
    fun `should return TaskExpired if current time is after end time`() {
        ...
        // THEN
        assertEquals(OwnershipChallengeResult.TaskExpired, result.returning)
    ...
    fun `should return InvalidBrawler if brawlerId does not match`() {
        ...
        // THEN
        assertEquals(OwnershipChallengeResult.InvalidBrawler, result.returning)
        assertIs<OwnershipChallengeUpdateEvent.VerificationFailed>(result.event)
    }
    ...
    fun `should return InvalidEventType if event type mismatches`() {
        ...
        // THEN
        assertEquals(OwnershipChallengeResult.InvalidEventType, result.returning)
        assertIs<OwnershipChallengeUpdateEvent.VerificationFailed>(result.event)
    }
    ...
    fun `should return InvalidBotsAmount if bot count mismatches`() {
        ...
        // THEN
        assertEquals(OwnershipChallengeResult.InvalidBotsAmount, result.returning)
        assertIs<OwnershipChallengeUpdateEvent.VerificationFailed>(result.event)
    }
    ...
    fun `should return Success and VerificationCompleted for valid battle`() {
        ...
        // THEN
        assertEquals(OwnershipChallengeResult.Success, result.returning)
        assertIs<OwnershipChallengeUpdateEvent.VerificationCompleted>(result.event)
        assertEquals(challenge.id, (result.event as OwnershipChallengeUpdateEvent.VerificationCompleted).challengeId)
    }
